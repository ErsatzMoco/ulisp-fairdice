...
(defun one-die ()
	...
	(loop
		...
		(buf-clear *canvas-buf*) 
		(buf-draw-arr *canvas-buf* (aref *img-list* (random 6)) 0 0) 
		(make-particle-list *canvas-buf*) 
...
(defun two-dice ()
	...
	(loop
		...
		(scramble-particles 20 t nil)
		(buf-clear *canvas-buf*)
		(let* ((result (+ (random 11) 2)) (tb-entry (nth (- result 2) *dice-table*)) 
...
(defun sym-dice ()
	...
	(loop
		...
		(scramble-particles 20 t nil)
		(buf-clear *canvas-buf*) 
		(buf-draw-arr *canvas-buf* (aref *img-list* (+ 6 (random 6))) 0 1)
		(buf-draw-arr *canvas-buf* (aref *img-list* (+ 12 (random 6))) 8 9)
		(make-particle-list *canvas-buf*) 
		(prep-converge-particles 10) 
		(scramble-particles 10 nil nil)  
		(scramble-particles 10 t t)
